...
class ReportController extends Controller {
	...
	public function index($id)
	{
		...
		if(Auth::user()->hasRole('County Lab Coordinator'))
			$subCounties = County::find(Auth::user()->tier->tier)->subCounties->pluck('name', 'id');
		//	Get all facilities
		$facilities = array();
		if(Auth::user()->hasRole('Sub-County Lab Coordinator'))
			$facilities = SubCounty::find(Auth::user()->tier->tier)->facilities->pluck('name', 'id');
		//	Declare variables
		$sdps = [];
        $sdp =NULL;
		$site = NULL;
		...
		if(Input::get('sub_county'))
		{
			...
			$facilities = SubCounty::find($sub_county)->facilities->pluck('name', 'id');
		}
		if(Input::get('county'))
		{
			$jimbo = Input::get('county');
			$subCounties = County::find($jimbo)->subCounties->pluck('name', 'id');
		}
		//	Get sdps
		$from = Input::get('from');
		if(!$from)
	...
	{	
		...
		foreach ($columns as $column)
		{
			$domain = array_merge($domain, $column->questions()->where('score', '!=', '0')->pluck('id'));
			foreach ($column->questions as $question) 
			{
				if($question->answers->count()>0)
				{
	...
	public function ptSdp()
	{
		...
		//	Get sdps
		$sdps = array_unique($checklist->surveys->pluck('sdp_id'));
		//	Periods
		$periods = array('Baseline', 'Quarter 1', 'Quarter 2', 'Quarter 3', 'Quarter 4');
	...
	public function logRegion()
	{
		...
		$regions = array();
		$subs = array();
		$facility = $checklist->surveys->pluck('facility_id');
	...
	*/	
	public static function getMonths($from, $to, $htc  = null)
	{
		...
		$allDates = array();
		if($htc)
			$allSurveyDates = $surveys->pluck('data_month');
		else
			$allSurveyDates = $surveys->pluck('date_submitted');
	...
	*/
	...
	public function dashboard()
	{
		...
		$sites = array();
		if(Auth::user()->hasRole('Sub-County Lab Coordinator'))
			$sites = SubCounty::find(Auth::user()->tier->tier)->facilities->pluck('name', 'id');
		$sdps = array();
		//	Declare variables
        $sdp =NULL;
		$site = NULL;
	...
	*/
	public function sdpsTitleN($chkId, $jimbo = NULL, $sub_county = NULL, $site = NULL, $sdp = NULL, $from = NULL, $to = NULL)
	{
		...
		else
		{
			...
			$fsdps = FacilitySdp::pluck('id')->unique();
		}
		$surveys = Survey::whereIn('facility_sdp_id', $fsdps)->whereBetween('date_submitted', [$from, $to])->where('checklist_id', $chkId)->pluck('facility_sdp_id');
		$n = count($surveys);
		$sdps = $sdps->unique('id');
