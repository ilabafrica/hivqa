...
class SurveyController extends Controller {
	...
	public function create($id)
	{
		...
		$algorithms = Algorithm::pluck('name', 'name');
		//	Get list of affiliations
		$affiliations = Affiliation::pluck('name', 'name');
		//	Get list of audit-types
		$auditTypes = AuditType::pluck('name', 'name');
		//	Get list of test kits
		$kits = TestKit::pluck('name', 'name');
	...
	public function listing($id)
	{
		...
		if($county || $subCounty)
		{
			if($subCounty)
			{
				$facilities = SubCounty::find($subCounty)->facilities->pluck('id');
				$fsdps = FacilitySdp::whereIn('facility_id', $facilities)->pluck('id');
			}
			else
			{
				$subCounties = County::find($county)->subCounties->pluck('id');
		...
		else
		{
			...
			$subCounties = SubCounty::whereIn('county_id', $cnts)->pluck('id');
			$facilities = Facility::whereIn('sub_county_id', $subCounties)->pluck('id');
			$fsdps = FacilitySdp::whereIn('facility_id', $facilities)->pluck('id');
		}
	...
	public function collection($id)
	{
		...
		if($county || $subCounty)
		{
			if($subCounty)
			{
				$fsdps = FacilitySdp::whereIn('facility_id', $subCounty->facilities->pluck('id'))->pluck('id');
			}
			else
			{
				$facilities = Facility::whereIn('sub_county_id', $county->subCounties->pluck('id'))->pluck('id');
				$fsdps = FacilitySdp::whereIn('facility_id', $facilities)->pluck('id');
			}
		}
		if($fsdps)
	...
	public function api($id)
	{
		...
		if($checklist->name == 'M & E Checklist')
			...
			$checklist_id = 290574; //207684;//205946;//94529;
    ...
    */
    ...
    public function process($data, $checklist)
    {
        ...
        foreach ($data as $key => $value)
        {
			...
			var_dump($sdpId);
			var_dump( $value["_id"]);
			$facility_sdp_id = FacilitySdp::where('facility_id', $facility_id) ->where('sdp_id', $sdpId)->first()->id;
			//dd($facility_sdp_id);
						
			/*
			$tierName = NULL;
			if(isset($value['facilitylevel']))
			{
				$string = $value['facilitylevel'];
				
				if (preg_match('#[0-9]#',$string))
				{
					$split = $this->first_number($value['facilitylevel']);
			...
			$facility_sdp_id = FacilitySdp::where('facility_id', $facility_id)->where('sdp_id', $sdpId)->first()->id;//where('sdp_tier_id', $tier)->first()->id;
			*/
    ...
    */
	...
	public function editSdp($id)
	{
		...
		$sdps = Sdp::pluck('name', 'id');
		//	Get audit_types
		$audit_types = AuditType::pluck('name', 'name');
		//	Get test_kits
		$test_kits = TestKit::pluck('name', 'name');
		//	Get algorithms
		$algorithms = Algorithm::pluck('name', 'name');
		//	Get affiliations
		$affiliations = Affiliation::pluck('name', 'name');
