...
class Checklist extends Model implements Revisionable {
	...
	*/
	...
	/**
	*
	*	Function to load counties with data for select pluck in views
	*
	*/
	public function countiesWithData()
	{
		$counties = [];
		$cIds = [];
		foreach ($this->surveys()->pluck('facility_sdp_id') as $value => $key)
		{
		    $scIds = [];
		    if(Facility::find(FacilitySdp::find($key)->facility_id))
		...
		$counties = County::whereIn('id', $cIds)->pluck('name', 'id');
		return $counties;
	}
	/**
	...
	public function programatic($percentage, $kit, $fsdp, $sdp = null, $site = NULL, $sub_county = NULL, $jimbo = NULL, $year = 0, $month = 0, $date = 0, $from = null, $to = null, $point = null)
	{
		...
		if($sdp || $site)
			...
			$fsdps = Sdp::find($fsdp)->facilitySdp->pluck('id');
	...
	public function level($lId, $county = NULL, $sub_county = NULL, $facility = NULL, $fsdp = NULL, $from = NULL, $to = NULL, $year = 0, $month = 0, $date = 0)
	{
		...
		$counter = 0;
		$fsdps = $this->fsdps($this->id, $county, $sub_county, $facility, $fsdp, $from, $to, $year, $month, $date)->pluck('facility_sdp_id');
		// $fsdps = array_filter($fsdps);		
		if($lId)
		{
		...
		else
		{
	        ...
	        $surveys = $surveys->pluck('id');
	        $total_counts = count($surveys);
	        $questions = SurveyQuestion::whereIn('survey_id', $surveys)->whereNotIn('question_id', $unwanted)->whereIn('question_id', DB::table('question_responses')->pluck('question_id'))->pluck('id');
	        $dbs = SurveyQuestion::whereIn('survey_id', $surveys)->where('question_id', $notapplicable)->pluck('id');
	        $na = SurveyData::whereIn('survey_question_id', $dbs)->where('answer', '0')->count();
	        $calculated_points = SurveyData::whereIn('survey_question_id', $questions)->whereIn('answer', Answer::pluck('score'))->sum('answer');
	        //  Begin processing
	        if($na>0)
        ...
        if($county || $sub_county || $site || $sdp)
        {
            if($sub_county || $site || $sdp)
            {
                ...
                {                                
                    ...
                    {
                    	$fsdps = Facility::find($site)->facilitySdp->pluck('id');
                    }
                }
                else
                {
                    $fsdps = FacilitySdp::whereIn('facility_id', SubCounty::find($sub_county)->facilities->pluck('id'))->pluck('id');
                }
            }
            else
            {
            	$fsdps = FacilitySdp::whereIn('facility_id', Facility::whereIn('sub_county_id', County::find($county)->subCounties->pluck('id'))->pluck('id'))->pluck('id');
            }
        }
        if(count($fsdps)>0)
	...
	public function regions($county = NULL, $sub_county = NULL)
	{
		...
		{
			if($sub_county)
				$regions = SubCounty::find($sub_county)->facilities->pluck('name', 'id');
			else
				$regions = County::find($county)->subCounties->pluck('name', 'id');
		}
		else
		{
	...
	*/
		...
		if($jimbo || $sub_county || $site || $sdp)
		{
			if($sub_county || $site || $sdp)
			{
				if($site || $sdp)
				{
					...
					{
						$sdps = $facility->facilitySdp->pluck('id');
						$title = $facility->name;
						$fsdps = $sdps;
					}
				}
				else
				{
					...
					$sdps = FacilitySdp::whereIn('facility_id', SubCounty::find($sub_county)->facilities->pluck('id'))->pluck('sdp_id');
					$fsdps = FacilitySdp::whereIn('facility_id', SubCounty::find($sub_county)->facilities->pluck('id'))->pluck('id');
				}
			}
			else
			{
				...
				$sdps = FacilitySdp::whereIn('facility_id', Facility::whereIn('sub_county_id', County::find($jimbo)->subCounties->pluck('id'))->pluck('id'))->pluck('sdp_id');
				$fsdps = FacilitySdp::whereIn('facility_id', Facility::whereIn('sub_county_id', County::find($jimbo)->subCounties->pluck('id'))->pluck('id'))->pluck('id');
			}
		}
		else
		{
			$title = 'Kenya';
			$sdps = FacilitySdp::pluck('sdp_id');
			$fsdps = FacilitySdp::pluck('id');
		}
		$surveys = $this->surveys()->whereIn('facility_sdp_id', $fsdps);
		if($this->id == Checklist::idByName('HTC Lab Register (MOH 362)'))
			...
			$surveys = $surveys->whereBetween('date_submitted', [$from, $to]);
		$surveys = $surveys->pluck('facility_sdp_id');
		$n = count($surveys);
	...
	public function regionalAgreement($percentage, $kit, $county = NULL, $sub_county = NULL, $facility = NULL, $from = NULL, $to = NULL)
	{
		...
		$fsdps = $this->sdpsTitleN($county, $sub_county, $facility, NULL, $from, $to)['sdps'];// fsdps($this->id, $county, $sub_county, $facility, NULL, $from, $to)->pluck('facility_sdp_id');
		$total_sites = count($fsdps);
		foreach ($fsdps as $fsdp)
		{
			if($facility)
