...
class Section extends Model implements Revisionable {
	...
	public function spider($sdp = NULL, $site = NULL, $sub_county = NULL, $county = NULL, $from = NULL, $to = NULL, $year = 0, $month = 0, $date = 0)
	{
        ...
        //  Check dates
        $fsdps = Checklist::find($checklist)->fsdps($checklist, $county, $sub_county, $site, $sdp, $from, $to, $year, $month, $date)->pluck('facility_sdp_id');
        ...
        $percentage = 0.00;
        //  Begin processing
        $questions = SurveyQuestion::whereIn('survey_id', $surveys)->whereNotIn('question_id', $unwanted)->whereIn('question_id', $this->questions->pluck('id'))->pluck('id');
        ...
        if($total_counts>0)
        {
	        if($na>0 && in_array($notapplicable, $this->questions->pluck('id')))
	...
	public function snapshot($county = null, $sub_county = null, $site = null, $sdp = null, $from = NULL, $to = NULL)
	{
        ...
        $total_counts = count($fsdps);
        $surveys = $fsdps->pluck('id');
        $sq = SurveyQuestion::whereIn('survey_id', $surveys)->whereIn('question_id', $this->questions->pluck('id'))->pluck('id');
        $calculated_points = SurveyData::whereIn('survey_question_id', $sq)
        							->whereIn('survey_data.answer', Answer::pluck('score'))
        							->sum('answer');
	...
	public function quarter($period)
	{
		...
		if($period === 'Baseline')
		{
			$survey_sdp_ids = SpirtInfo::pluck('survey_sdp_id');
		...
		else
		{
			...
			$survey_sdp_ids = SpirtInfo::join('survey_sdps', 'survey_sdps.id', '=', 'survey_spirt_info.survey_sdp_id')
										...
										->pluck('survey_spirt_info.survey_sdp_id');
			$counter = count($survey_sdp_ids);
		}
		if($this->isScorable())
		{
	...
	*/
    ...
    public function level($checklist, $response, $county = NULL, $sub_county = NULL, $site = NULL, $sdp = NULL, $from = NULL, $to = NULL)
    {
        ...
        foreach ($fsdps->get() as $key => $value)
        {
            ...
            $percentage = 0.00;
            $sqtns = $value->sqs()->whereIn('question_id', $this->questions->pluck('id'))    //  Get questions belonging to the section
                                  ->join('survey_data', 'survey_questions.id', '=', 'survey_data.survey_question_id')
                                  ->whereIn('survey_data.answer', array_filter(Answer::pluck('score')));
